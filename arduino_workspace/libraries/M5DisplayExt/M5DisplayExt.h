...
public:
  ...
  void drawBmpFileSprite(TFT_eSPI *sprite, fs::FS &fs, const char *path,
...
// void drawJpgFileSprite(TFT_eSPI* sprite, fs::FS &fs, const char *path,
...
//               jpeg_div_t scale = JPEG_DIV_NONE);
  ...
  void drawPngFileSprite(TFT_eSPI *sprite, fs::FS &fs, const char *path,
                         uint16_t x = 0, uint16_t y = 0, uint16_t maxWidth = 0,
...
private:
  void need_boost(void) {
    if (cb_needboost != NULL) {
      cb_needboost();
    }
  };
};
